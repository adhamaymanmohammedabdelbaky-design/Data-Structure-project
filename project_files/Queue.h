...
private:
    ...
    // Helper function to clear the queue (used in destructor and assignment operator)
    void clearQueue() {
        Node* temp = front;
        while (temp != NULL) {
            Node* delptr = temp;
...
public:
    ...
    void updateWaitingTimes(int currentTime) {
        Node* current = front;
        while (current != NULL) {
            current->data.calculateWaitingTime(currentTime); // Update waiting time for each customer
